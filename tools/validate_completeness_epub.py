#!/usr/bin/env python3
"""Independently validate the proof-systems/completeness EPUB archive."""
...
CHAPTERS = [f"chapter-{number:02d}.xhtml" for number in range(1, 8)]
...
def main() -> None:
    ...
    with zipfile.ZipFile(args.epub) as archive:
        ...
        require(
            spine_ids == ["title", *(f"chapter-{number:02d}" for number in range(1, 8))],
        ...
        expected_units = [f"OLP-{number:04d}" for number in range(56, 138)]
        require(sorted(unit_anchors) == expected_units, "Source-unit anchors do not cover OLP-0056..OLP-0137")
    ...
    receipt = {
        "schema": "openlogic-ps-Arab-PK-proof-systems-completeness-epub-independent-validation/1",
        "status": "accepted",
        "inputs": {
