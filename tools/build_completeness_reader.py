"""Build the isolated OLP-0056..OLP-0137 Pashto proof-systems/completeness reader.
...
EXPECTED_IDS = [f"OLP-{number:04d}" for number in range(56, 138)]
DRIVER_IDS = {
    "OLP-0056",
    "OLP-0063",
    "OLP-0069",
...
EXTERNAL_REFERENCES = {
    ...
    "fol:syn:sem:prop:entails-unsat": {
        "title": "معنايي استلزام او د صدق نۀ وړتيا قضيه",
        "source": "semantic-notions.tex",
        "line": 62,
        "occurrences": 1,
    },
...
TOKENS: dict[str, tuple[str, str, str, str]] = {
    ...
    "value": ("قيمت", "قيمتونه", "قيمت", "قيمتونو"),
...
def skip_tex_space(text: str, pos: int) -> int:
    """Skip inter-argument whitespace, comments and alignment-anchor boundaries."""
    while pos < len(text):
        if text[pos].isspace():
            pos += 1
            continue
...
def replace_tagprobs(text: str, stats: Counter) -> str:
    ...
    while True:
        ...
        if not text.startswith(begin_marker, pos):
            ...
            raise ValueError(
                f"tagprob is not followed by a prob environment near {context!r}"
            )
...
def resolve_profile_conditionals(text: str) -> tuple[str, Counter]:
    """Resolve every selective-typesetting construct used by this reader."""
...
def expand_tokens(text: str) -> str:
    ...
    def replace_token(match: re.Match[str]) -> str:
        ...
        # The reader does not need an oblique switch: OLP-0056..0137 contains
...
def prepare_unit(
    ...
    if "\\psOblique" in body:
        raise ValueError("unexpected psOblique call in proof-systems/completeness reader")
...
def reference_inventory(texts: list[tuple[dict, str]]) -> dict:
    ...
    return {
        ...
        "note": (
            ...
            "selected out-of-volume references are rendered as named links to the frozen source."
        ),
    }
...
def main() -> None:
    ...
    if [row["unit_id"] for row in selected] != EXPECTED_IDS:
        raise ValueError("manifest does not contain the exact OLP-0056..OLP-0137 sequence")
    ...
    if external_reference_counts != expected_external_reference_counts:
        raise ValueError(
            "selected external-reference counts differ from the audited occurrences: "
    ...
    preamble = ROOT / "tools" / "completeness-reader-preamble.tex"
    ...
    record = {
        "schema": "openlogic-ps-Arab-PK-proof-systems-completeness-reader-inputs/1",
        "status": "prepared",
        ...
        "unit_range": {"first": "OLP-0056", "last": "OLP-0137", "count": len(selected)},
